...
- Prompts for directory mounts BEFORE read-only mode for each container.
- Warnings about DB container read-only pitfalls.
- Basic collision handling for container names.

Note: Some e-commerce images may exit if they encounter a configuration error
or if their main process fails. See explanation at the end.
"""
...
# -------------------------------------------------
# 5. Deploy Web Stack (DB + Web + Optional WAF) in a New Order
# -------------------------------------------------

def deploy_web_stack():
    """
    A more robust deployment flow:
      1. Ask user if they want containerized DB or native OS DB.
      2. If container DB, deploy it first (mount dirs, then ask read-only).
      3. Deploy the web app container (mount dirs, then ask read-only).
      4. Optionally deploy ModSecurity WAF last.
      5. Offer integrity checking at the end.
    """
    ...
    if dockerized_db:
        # Prompt for DB container name, default "web_db"
        default_db_name = "web_db"
        print("=== Database Container Setup ===")
        db_container = prompt_for_container_name(default_db_name)
        
        print("[NOTE] A database container typically needs write access. If you choose read-only, you must mount all needed writable directories (e.g., /var/lib/mysql).")
        
        # Prompt for directories to mount
        volume_opts_db = []
        print("Enter the directories you want to mount into the DB container, one at a time.")
        print("For example, /var/lib/mysql if you want to use host storage. Leave blank to finish.")
        while True:
        ...
        db_user = "root"  # typical root user in default mariadb
    ...
    volume_opts_web = []
    print("Enter the directories you want to mount into the web container, one at a time.")
    print("Leave this blank and press Enter when you're finished.")
    while True:
        dir_input = input("Directories to mount into the web container (blank to finish): ").strip()
        if not dir_input:
            break
    ...
    env_vars = []
    if dockerized_db:
        # If using container DB, pass DB connection info to the web container
        env_vars.extend([
        ...
        # For native OS DB, prompt the user for connection details
    ...
    # Build docker run command for the web container
    cmd = [
        "docker", "run", "-d",
...
# -------------------------------------------------
# 6. Optional: Additional Commands or Integrity Menus
# -------------------------------------------------
...
def interactive_menu():
    ...
    print("2. Run Continuous Integrity Check on a Container")
...
def main():
    parser = argparse.ArgumentParser(
        description="CCDC OS-to-Container & Integrity Tool with re-ordered web stack deployment (DB, Web, WAF), directory-mount prompts, and container name collision handling."
    )
