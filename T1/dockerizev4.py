...
Updated script with re-ordered container deployment:
- First ask for DB type (native vs container).
- If container DB, deploy it first.
- Then deploy the web app container (e.g., PrestaShop), prompting for DB env vars.
- Finally, optionally deploy ModSecurity WAF.

Prompts user for read-only mode for each container and allows repeated directory inputs.
"""
...
def deploy_web_stack():
    ...
    A more robust deployment flow:
      ...
      2. If container DB, deploy it first (prompt for read-only).
      3. Deploy the web app container (e.g., PrestaShop), prompting for DB env vars and read-only choice.
      4. Prompt for repeated directory mounts.
      5. Optionally deploy ModSecurity WAF last, prompting for read-only choice as well.
    """
    ...
    if dockerized_db:
        db_container = input("Enter a name for the DB container (default 'mariadb_db'): ").strip() or "mariadb_db"
    ...
    # Web container name
    service_container = input("Enter a name for the service container (default 'web_container'): ").strip() or "web_container"
    ...
    env_vars = []
    if dockerized_db:
        # If using container DB, we pass DB connection info to the web container
        env_vars.extend([
    ...
    # Step 4: Prompt for repeated directory mounts
    ...
    # Step 5: Optionally deploy a ModSecurity WAF
    ...
    # Finally, ask if user wants continuous integrity checking
...
def main():
    parser = argparse.ArgumentParser(
        description="CCDC OS-to-Container & Integrity Tool with re-ordered web stack deployment (DB, Web, WAF)."
    )
