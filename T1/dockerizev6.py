...
- Prompts for read-only mode for each container.
- Repeated directory inputs for mounting (one at a time).
- Default DB container name changed to "web_db".
- Basic collision handling for container names.

Note: Some e-commerce images may exit if they encounter a configuration error or if their main process fails. See explanation below.
"""
...
def deploy_web_stack():
    ...
    A more robust deployment flow:
      ...
      4. Prompt for repeated directory mounts (one at a time).
    ...
    if dockerized_db:
        # Prompt for DB container name, default changed to "web_db"
...
def main():
    parser = argparse.ArgumentParser(
        description="CCDC OS-to-Container & Integrity Tool with re-ordered web stack deployment (DB, Web, WAF) and container name collision handling."
    )
